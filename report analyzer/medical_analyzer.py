...
# Configure Google Generative AI API (ensure your API key is correctly set)
try:
    genai.configure(api_key="AIzaSyABLnzAACowPS1tIry_7X3ubnohxJojpRQ") # REPLACE THIS WITH YOUR ACTUAL API KEY
except Exception as e:
    print(f"Failed to configure Gemini API. Make sure your API key is set correctly. Error: {e}")
    exit()

gemini_llm = genai.GenerativeModel("gemini-1.5-flash") # Or "gemini-2.5-flash" if you prefer
...
def analyze_pdf_report(pdf_content_bytes: bytes, filename: str) -> dict:
    ...
    try:
        ...
        **Ensure all commas, brackets, and quotes are correctly placed according to JSON syntax rules.**
        ...
        response = gemini_llm.generate_content(prompt, request_options={"timeout": 600})
